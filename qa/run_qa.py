...
def main():
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        predictions, probabilities = postprocess_qa_predictions(
            examples=examples,
            features=features,
        ...
        for preds in predictions:
            references.append({"id": preds['id'], "answers": ex_ids[preds['id']][answer_column_name]})
        return EvalPrediction(predictions=formatted_predictions, label_ids=references), probabilities

    metric = load_metric("squad_v2" if data_args.version_2_with_negative else "squad")

    def compute_metrics(p: EvalPrediction):
        #changed to evaluate for multiple answer spans per document
        #picks highest scoring (F1) span per doc for final evaluation
        preds_dict = {}
        for preds in p.predictions:
            if preds['id'] not in preds_dict:
                preds_dict[preds['id']] = [preds]
            else:
                preds_dict[preds['id']].append(preds)
        final_preds = []
        final_references = []
        id2ref = {}
        for ref in p.label_ids:
            if ref['id'] not in id2ref:
        ...
        for k,v in preds_dict.items():
            if len(v) > 1:
                ...
                max_pred = v[0]
                for value in v:
                    temp = metric.compute(predictions=[value], references=[id2ref[value['id']]])
                    if temp['f1'] > max_f1:
                        max_f1 = temp['f1']
                        max_pred = value
