    ...
    for example_index, example in enumerate(tqdm(examples)):
        ...
        for pred in predictions[1:20]:
            ...
            for start_tok, end_tok in answer_pairs:
                # if (pred["start_index"] > end_tok or pred["end_index"] < start_tok)  and pred["score"].item() > predictions[0]['score'].item()/2:
        ...
        if not version_2_with_negative:
            for preds_index, preds in enumerate(new_predictions):
                all_predictions.append({"id": example["id"], "prediction_text": preds["text"]})
                #all_predictions[example["id"]] = preds["text"]
                probabilities[example["id"]] = probs[preds_index]
        else:
            ...
            probabilities[example["id"]] = probs[0]
